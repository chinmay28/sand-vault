...
func TestChangePasswordRotatesTheKeyThatOpensTheParts(t *testing.T) {
	...
	entry, _, err := v.Upload(ctx, MainScope, "/", "rotated.txt", payload, UploadOptions{})
	if err != nil {
...
func TestDeferredMigrationKeepsFilesReadableAndResumes(t *testing.T) {
	...
	first, _, err := v.Upload(ctx, MainScope, "/", "one.txt", []byte("the first file"), UploadOptions{})
	if err != nil {
		t.Fatalf("Upload: %v", err)
	}
	second, _, err := v.Upload(ctx, MainScope, "/", "two.txt", []byte("the second file"), UploadOptions{})
	if err != nil {
		t.Fatalf("Upload: %v", err)
	}
...
func TestMigrationLeavesUnreadableFilesOnTheOldKey(t *testing.T) {
	...
	readable, _, err := v.Upload(ctx, MainScope, "/", "reachable.txt", []byte("this one is fine"), UploadOptions{})
	if err != nil {
		t.Fatalf("Upload: %v", err)
	}
	stranded, _, err := v.Upload(ctx, MainScope, "/", "stranded.txt", []byte("this one is not"), UploadOptions{})
	if err != nil {
		t.Fatalf("Upload: %v", err)
	}
...
func TestDeletingTheLastFileOnAnOldKeyDropsIt(t *testing.T) {
	...
	entry, _, err := v.Upload(ctx, MainScope, "/", "doomed.txt", []byte("not for long"), UploadOptions{})
	if err != nil {
		t.Fatalf("Upload: %v", err)
	}
...
func TestChangePasswordRejectsTheWrongCurrentPassword(t *testing.T) {
	...
	if _, _, err := v.Upload(ctx, MainScope, "/", "untouched.txt", []byte("safe"), UploadOptions{}); err != nil {
		t.Fatalf("Upload: %v", err)
	}
...
func TestRecoveryAfterAnInterruptedPasswordChange(t *testing.T) {
	...
	if _, _, err := original.Upload(ctx, MainScope, "/", "before.txt", old, UploadOptions{}); err != nil {
		t.Fatalf("Upload: %v", err)
	}
	...
	if _, _, err := original.Upload(ctx, MainScope, "/", "after.txt", fresh, UploadOptions{}); err != nil {
		t.Fatalf("Upload: %v", err)
	}
...
func TestBackupsAreReplacedAfterAPasswordChangeEvenIfThePushFailed(t *testing.T) {
	...
	if _, _, err := v.Upload(ctx, MainScope, "/", "indexed.txt", []byte("in the index"), UploadOptions{}); err != nil {
		t.Fatalf("Upload: %v", err)
	}
	v.AwaitBackupSync()
...
func TestMigrationProgressReportsEveryFile(t *testing.T) {
	...
	for _, name := range []string{"a.txt", "b.txt", "c.txt"} {
		if _, _, err := v.Upload(ctx, MainScope, "/", name, []byte(name), UploadOptions{}); err != nil {
